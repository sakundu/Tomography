...
class ImageDataset_sample(Dataset):
    ...
    def update_data(self, data_file:str):
        ...
        for design_id, config_list in tqdm(self.config_list.items()):
            feature_file = f"{self.data_dir}/features/run_{design_id}.npy"
            feature = np.load(feature_file)
            self.features[design_id] = feature
            self.encodings[design_id] = {}
            for config_id in config_list:
                encode_prefix = f"{self.data_dir}/run_{design_id}/run_{design_id}"
                encode_file = f"{encode_prefix}_{config_id}.npy"
                if self.is_int:
                    encode_file = encode_file.replace('.npy', '_int.npy')
...
class BestConfigDataset(Dataset):
    def __init__(self, dbscan_file:str, data_dir:str, run_id:int,
                 configs:List[List[int]], total_number_config:int = 15) -> None:
        self.dbscan_file:str = dbscan_file
        self.data_dir = data_dir
        self.run_id:int = run_id
        self.total_number_config:int = total_number_config
        ...
        self.data_configs = configs
        num_sample = len(self.data_configs)
        self.data = [(x, y) for x in range(num_sample) for y in range(num_sample) if x != y]
        self.encodings = []
        self.gen_condings()

    def gen_condings(self):
        for config in self.data_configs:
            encode = get_encoding(self.boxes, config, self.size_x, self.size_y, self.total_number_config)
    ...
    def temp_get_item(self, idx:int) -> torch.Tensor:
        ...
        # print(f"Time taken for encoding generation: {time.time() - start_time}")
        
        # start_time = time.time()
        encode1 = torch.tensor(encode1, dtype=torch.float32)
        encode2 = torch.tensor(encode2, dtype=torch.float32)
        # print(f"Time taken for tensor generation: {time.time() - start_time}")
        
        # start_time = time.time()
        encode1 = encode1.permute(2, 0, 1)
        encode2 = encode2.permute(2, 0, 1)
        # print(f"Time taken for permute: {time.time() - start_time}")
        
        # start_time = time.time()
        data = torch.cat((self.feature, encode1, encode2), dim=0)
        # print(f"Time taken for concat: {time.time() - start_time}")
        
        # start_time = time.time()
        data = self.resize(data)
        # print(f"Time taken for resize: {time.time() - start_time}")
        return data
    
    def __getitem__(self, idx:int) -> torch.Tensor:
        e1_id = self.data[idx][0]
        e2_id = self.data[idx][1]
        data = torch.cat((self.feature, self.encodings[e1_id], self.encodings[e2_id]), dim=0)
        return data