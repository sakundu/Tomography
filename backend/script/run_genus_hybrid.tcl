...
if { $use_cache == "1" && [file exists $cached_netlist] && [file exists $cached_sdc] } {
  puts "Exiting early because cached netlist and sdc exist."
  puts "Cached netlsit: $cached_netlist"
  puts "Cached sdc: $cached_sdc"
  exit
}

if {[info exist ::env(PHY_SYNTH)] && $::env(PHY_SYNTH) == 1} {
    read_mmmc ${proj_dir}/util/mmmc_iSpatial_setup.tcl
}
...
#################################################
set_db init_hdl_search_path $searchdir 
source ${proj_dir}/rtl/${DESIGN}/rtl_list.tcl

foreach rtl_file $rtl_all {
  if { $DESIGN != "mempool_group" } {
    read_hdl -sv $rtl_file
  } else {
    read_hdl -language sv -define TARGET_SYNTHESIS -define XPULPIMG=1 -define NUM_CORES=256 -define AXI_HIER_RADIX=20 -define AXI_DATA_WIDTH=256 -define NUM_GROUPS=4 -define RO_LINE_WIDTH=256 -define NUM_CORES_PER_TILE=4 -define L2_SIZE=400000 -define L2_BANKS=4 -define SEQ_MEM_SIZE=1024 -define BANKING_FACTOR=4 $rtl_file
  }
}
...
if { $DESIGN != "mempool_group" || $DESIGN != "ariane" || $DESIGN != "bsg_chip" } {
  define_test_signal -function shift_enable -name SE_signal -active high \
      -default_shift_enable -create_port SE
